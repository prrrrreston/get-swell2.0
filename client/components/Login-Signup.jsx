...
function LoginSignup() {
  ...
  const handleLoginSubmit = async event => {
    ...
    const password = data.get('password');
    // Send the info to the database
    const serverResponse = await fetch('/verify/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    <Container component="main" maxWidth="xs">
      ...
      >
        <Box sx={{
          ...
          borderRadius: 15,
          color: 'white',
          backgroundColor: 'rgb(118,166,223)',
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
        ...
        <Avatar sx={{ m: 2, bgcolor: 'rgb(118,166,223)' }}>
          <AccountCircleIcon />
        </Avatar>
        ...
        <Box component="form" onSubmit={handleSignupSubmit} noValidate sx={{ mt: 1 }}>
          ...
          <Button
            ...
            fullWidth
            variant="contained"
            sx={{ mt: 3, mb: 2, backgroundColor: 'rgb(118,166,223)' }}
          >
            Create Account
          </Button>
        </Box>
